...
| **Peak FP32** | 180 TFLOPS | 60 TFLOPS | 90 TFLOPS | B200 (3x) |
| **Peak FP8** | ~9 PFLOPS | ~1.8 PFLOPS | ~3.6 PFLOPS | B200 (2.5x) |
...
**FP8 Tensor Performance.** B200 native FP8 tensors hit 720 TFLOPS (compared to H100's 182 TFLOPS equivalent when casting from TF32). For inference quantized to 8-bit, B200 is near-native speed. H100 requires casting overhead. FP4 (4-bit) performance: B200 reaches 1,440 TFLOPS (theoretical double of FP8). Real-world FP4 training is rare, but research projects benefit.
...
**NVLink 5.0.** B200 uses NVLink 5.0, up from NVLink 4.0 on H100. NVLink 5.0 bandwidth per link: 960 GB/s (vs 900 GB/s on H100). Scaling: 8x B200 in NVLink can aggregate 7.68 TB/s all-reduce bandwidth. Latency is 50% lower (thanks to NVIDIA's fabric optimizations). For distributed training with large all-reduce operations (8-64 GPUs), the latency reduction compounds to 20-30% wall-clock speedup.
...
**FP8 (8-bit floating point, inference quantization):** 720 TFLOPS per B200. For quantized models (common in production), B200 is near-peak throughput.

**FP16 (16-bit floating point, training):** 360 TFLOPS per B200. This is where H100 is 120 TFLOPS (or 1,457 TFLOPS on TF32, which isn't FP16).
...
For inference: 2.4-3.2x faster throughput (depends on model size and batch size). For training: roughly 2.4x faster due to 2.4x bandwidth. For FP8 operations: 3.95x faster (720 vs 182 TFLOPS).
