...
Resource requests versus limits work differently for GPUs than CPU/memory. Developers specify a single value: GPUs can't be throttled like CPU. A pod either has exclusive access to a GPU or it doesn't. Requesting GPU0 through GPU3 means those exact GPUs are reserved for the workload.
