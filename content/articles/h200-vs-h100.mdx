...
H200 vs H100 is the focus of this guide. H200: 141GB HBM3e. H100: 80GB HBM3. Same architecture, same bandwidth (3.35 TB/s). Memory-only upgrade: 76% more VRAM. RunPod: $3.59/hr (H200) vs $2.69 (H100) = 35% premium. Worth it for large models only.
...
| **Memory Bandwidth** | 3.35 TB/s | 3.35 TB/s | Tie |
...
| **Peak FP32 Throughput** | 60 TFLOPS | 60 TFLOPS | Tie |
| **NVLink (SXM)** | 900 GB/s per GPU | 900 GB/s per GPU | Tie |
| **TDP** | 700W | 700W | Tie |
| **Cloud Price/hr (RunPod)** | $2.69 (SXM) | $3.59 | H100 (35% cheaper) |
| **Cloud Price/hr (Lambda)** | $2.49 | Not listed | H100 (availability) |
...
Enhanced HBM3 with higher density. 141GB achieved with larger dies (12.8GB per die instead of 8GB). Memory clock: 10.6 GHz (nominal, slightly faster). Same 3.35 TB/s bandwidth, but with more capacity per GPU.

The key difference is capacity, not speed. Developers get 61GB extra VRAM. Same memory bus speed means compute throughput per token is unchanged. For models larger than 80GB, H200 eliminates multi-GPU requirements.
