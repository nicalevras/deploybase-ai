...
- Llama 3.1 70B: ~$0.59 per 1M input tokens, ~$0.79 per 1M output tokens
- Llama 3.1 8B: $0.12 per 1M input tokens, $0.18 per 1M output tokens
- Mixtral 8x7B: $0.12 per 1M input tokens, $0.18 per 1M output tokens
...
Cerebras targets production customers. Pricing operates on dedicated capacity model (minimum commitments). Public API pricing not published; production licensing required.

Estimated pricing: $2-5 per 1M tokens (inference-optimized models), significantly lower per-token on high-volume commitments.
