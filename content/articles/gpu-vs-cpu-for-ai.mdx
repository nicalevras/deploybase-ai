...
For AI, this architectural difference is decisive. Training Llama 2 70B requires matrix multiplications at 10^18 operations per second (exascale). A single CPU can't sustain 100 billion operations per second. A single H100 GPU hits 60 trillion FP32 operations per second (60 TFLOPS).
...
NVIDIA H100: 16,896 CUDA cores × 1.8 GHz = 60 trillion FP32 operations per second (peak, achieved).
...
H100: 60 TFLOPS (achieved) at 700W = 86 GFLOPS per watt.
...
GPU: 70B ops / 60 TFLOPS = ~1,200 milliseconds
...
- **H100 80GB SXM:** 60 TFLOPS FP32
- **Performance ratio:** 3.1x advantage to H100

Why the difference: Hopper architecture packs 144 streaming multiprocessors with 128 FP32 CUDA cores each (16,896 total). Ampere's A100 has 108 SMs × 64 cores = 6,912 FP32 cores.
