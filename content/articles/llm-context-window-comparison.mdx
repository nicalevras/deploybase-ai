...
Claude Opus 4.6 pricing: $15 per million input tokens ($0.015 per 1,000), $75 per million output tokens ($0.075 per 1,000). The same 100,000 token document costs $1.50 for input.

Gemini 2.5 Pro pricing: $2.50 per million input tokens ($0.0025 per 1,000), $10 per million output tokens ($0.01 per 1,000). A 100,000 token document costs $0.25 for input, 10x cheaper than Claude on input cost.

Llama 3.1 via Replicate: approximately $0.00015 per input token, $0.00075 per output token for 405B variant. A 100,000 token input costs $0.015, 100x cheaper than Claude input cost but requires self-hosting or using API providers with variable pricing.
...
The practical implication: A task processing 50 100K-token documents costs $75 with Claude Opus 4.6 ($1.50 per document) or $12.50 with Gemini 2.5 Pro ($0.25 per document), a 6x cost difference despite identical context handling capability.
...
Claude Opus 4.6: 1,000,000 input tokens = $15, amortized $1.50 per document.

Gemini 2.5 Pro: 1,000,000 input tokens = $2.50, amortized $0.25 per document.

Individual API calls for 10 separate documents:

Claude Opus: 10 x $1.50 = $15.00 (same cost).

Gemini 2.5 Pro: 10 x $0.25 = $2.50 (same cost).
...
Model pricing has declined dramatically as competition intensified. In 2023, Claude input pricing was $0.03 per 1,000 tokens. Current Claude Opus 4.6 pricing at $0.015 per 1,000 tokens represents 50% cost reduction in two years. For teams running inference at scale, GPU infrastructure costs also matter significantly. Check [GPU pricing comparison](/articles/runpod-gpu-pricing), [H100 vs H200 vs B200](/articles/h100-vs-h200-vs-b200), and [LLM API pricing comparison](/articles/llm-api-pricing-comparison-cost-per-million-tokens-all) for serving optimization.
