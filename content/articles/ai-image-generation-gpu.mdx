...
Ada generation, originally designed for gaming. 24GB GDDR6X memory. Peak throughput: 1.4 TFLOPS for FP32.
...
Downsides: GDDR6X memory has lower bandwidth (960 GB/s) than HBM (1,935+ GB/s). Power draw spikes to 575W. Multi-user isolation is nonexistent: one customer's inference jobs interfere with others.

Best for: Teams serving up to 10k images/day. Startups building image APIs. Anything under $500/month infrastructure budget.

### RTX 3090 ($0.22/hr)

Ampere generation, older than 4090. 24GB GDDR6X. Slightly slower: 1.39 TFLOPS FP32.
...
Ampere generation optimized for inference (not training). 48GB GDDR6 memory. Bandwidth 960 GB/s.
