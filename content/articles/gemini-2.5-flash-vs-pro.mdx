...
Gemini 2.5 Flash vs Pro is the focus of this guide. Flash: $0.075 input, $0.30 output per 1M tokens. Fast, cheap.

Pro: $1.25 input, $10 output. Slower, better reasoning.

Flash is roughly 17x cheaper on input, 33x cheaper on output. Pick Flash for chat, classification, summarization. Pick Pro when reasoning quality matters.

### Quick Comparison

| Metric | Flash | Pro |
|--------|-------|-----|
| Input Cost (per 1M tokens) | $0.075 | $1.25 |
| Output Cost (per 1M tokens) | $0.30 | $10 |
...
The headline difference is dramatic: Flash is 17x cheaper on inputs, 33x cheaper on outputs. But raw cost numbers don't tell the full story.

Assume a 2M token input (typical long document analysis) and 1M token output response:

**Flash Cost:**
- Input: (2M tokens / 1M) × $0.075 = $0.15
- Output: (1M tokens / 1M) × $0.30 = $0.30
- Total: $0.45 per request
...
One request costs $0.45 with Flash, $12.50 with Pro. Multiply that by 1,000 requests daily and the gap widens to $450 vs $12,500 per day.

However, if Flash requires two attempts due to reasoning failures, costs converge. A $0.45 × 2 = $0.90 vs $12.50 single attempt becomes the real equation. This is where user context matters.
...
Without caching:
- Flash: (100k × 20 × $0.075) / 1M = $0.15
- Pro: (100k × 20 × $1.25) / 1M = $2.50

With caching (first request loads cache, next 19 reuse):
- Flash: First request $0.0075, next 19 at 90% discount = $0.0075 + (100k × 19 × $0.0075) / 1M = $0.0075 + $0.01425 = $0.02175
...
When caching is unavailable or context changes per request, baseline pricing applies and the original 8-17x gap persists.
...
- Pro video API: Upload video, let Pro sample frames internally = estimated 50-60k tokens processed = $0.0625 input cost
...
**Customer support chatbots.** A SaaS company handles 500 support tickets daily. Each query averages 1k input, 500 output tokens. Flash costs $0.000225 per ticket; Pro costs $0.005625. Annual cost: Flash ~$41, Pro ~$1,026. Flash handles basic troubleshooting, account info, and policy questions without issue. Escalate tricky cases to humans.

**Content summarization.** A news aggregation platform ingests 100k articles daily, each 2k tokens. Summarize to 200 tokens. Flash: 100K × (2K × $0.075 + 200 × $0.30) / 1M = $21/day ≈ $630/month. Pro: 100K × (2K × $1.25 + 200 × $10) / 1M = $450/day ≈ $13,500/month. Flash owns this workload.

**Data classification.** A logistics company tags inbound shipments by cargo type, destination, urgency. Input: 500 tokens per shipment, output: 100 tokens (category + confidence). 10k shipments/day. Flash costs $0.675/day; Pro costs $16.25/day. Flash is the obvious choice.

### Pro: Best For

**Legal document analysis.** A law firm reviews acquisition contracts. The model must catch ambiguous clauses, flag liability risks, and cross-reference other agreements in the corpus. This requires multi-step reasoning. 50 contracts/month, 5k tokens input, 2k tokens output. Flash: ($0.075 × 5 + $0.30 × 2) / 1M × 50 = ~$0.45 per contract × 50 = $22.50. Pro: $12.50 × 50 = $625. Cost difference ($602.50) is trivial next to liability risk of low-quality analysis. Pro is mandatory.

**Academic research synthesis.** A researcher compiles findings from 20 papers into a thesis chapter. The model must reconcile conflicting methodologies, synthesize novel insights, and maintain academic rigor. Context: 100k input tokens (full papers), 5k output tokens (chapter draft). Flash: (100K × $0.075 + 5K × $0.30) / 1M = $0.0075 + $0.0015 = $0.009. Pro: (100K × $1.25 + 5K × $10) / 1M = $0.125 + $0.05 = $0.175. The cost difference is small either way — but Pro's reasoning quality difference justifies the ~$0.17 premium for academic work.
...
Flash's lower cost enables liberal retry strategies. If a request fails, retry immediately at minimal cost ($0.45 × 2 = $0.90 total). Pro's cost ($12.50) makes retries expensive.
...
- Cost with caching: 50 FAQs × 1k tokens × (1 initial load + 99 cached accesses × 0.1 cost) = 50 × 1k × 10 = $0.45/day
- Cost without caching: 50 × 1k × 100 × $0.075 = $3.75/day
- Savings: $7.05/day = $2,573/year
...
- Flash: $0.000015 + $0.000045 = $0.00006
- Pro: $0.00025 + $0.0015 = $0.00175
- Pro costs 29x more

**Long summarization (5k input, 1k output tokens):**
- Flash: $0.000375 + $0.0003 = $0.000675
- Pro: $0.00625 + $0.01 = $0.01625
- Pro costs 24x more

**Deep analysis (20k input, 3k output tokens):**
- Flash: $0.0015 + $0.0009 = $0.0024
- Pro: $0.025 + $0.03 = $0.055
- Pro costs 23x more

The ratio stays consistent: Pro is 23-29x pricier, depending on output volume. This is the decision threshold. If retry rates with Flash exceed 5-8%, switch to Pro. If Flash succeeds consistently, its cost advantage is too large to ignore.
...
Retry cost with Flash ($0.45 × 2 = $0.90) is far below Pro's single attempt ($12.50). Even at 3 retries, Flash costs $1.35 vs Pro's $12.50. The decision depends on task type. For knowledge retrieval (4% accuracy gap), Flash's retry cost is easily acceptable. For complex reasoning (19% gap), Pro's upfront premium is justified.
