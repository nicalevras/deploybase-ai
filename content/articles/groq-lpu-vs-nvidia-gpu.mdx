...
NVIDIA GPUs contain thousands of small cores with massive parallelism. The H100 Tensor Core GPU has 14,080 CUDA cores working in parallel. This parallelism excels at matrix multiplication, convolution, and other data-parallel operations. But for inference where each token depends on the previous one, much of that parallelism sits idle.
