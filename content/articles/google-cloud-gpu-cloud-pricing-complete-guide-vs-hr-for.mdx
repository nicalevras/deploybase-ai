...
- On-demand: ~$3.30/hr per GPU
- Total: ~$26.40/hr for 8 GPUs
- Memory: 1152GB RAM
- CPUs: 104 vCPUs

A3 pricing is competitive among hyperscalers. However, supply can be limited and availability fluctuates. Instances may take longer to provision than A2.

**Cost comparison (per H100 GPU):**
- Google Cloud A3: $3.30/hr per GPU
- AWS p5.48xlarge: ~$12.25/hr per GPU (8-GPU instance, $98/hr total)
- RunPod: $2.69/hr
- Vast.AI: $2.00-3.50/hr

Google Cloud is the cheapest hyperscaler for H100 on-demand. Specialized providers remain cheaper overall.
...
TPU v5e at $2.50/hr is exceptional value for TensorFlow. If developers can use TPU, do it. Cost per compute unit is lowest available.

### 2. Right-size for single workloads

Single A100 on a2-highgpu-1g ($3.67/hr) avoids paying for 8 GPUs when only one is needed. If developers don't need parallelism, buy minimal resources.
...
For single A100, Google Cloud allows flexible single-GPU rental at $3.67/hr while AWS requires buying an 8-GPU node at $30.60/hr. For multi-GPU instances, AWS is cheaper ($30.60 vs $35.20 for 8x A100). For H100, GCP's a3-megagpu is cheapest among hyperscalers at $3.30/GPU. TPU work is dramatically cheaper on Google Cloud.
...
Single A100 (a2-highgpu-1g) at $3.67/hr for single-GPU workloads where you'd otherwise pay for 8 GPUs on AWS/Azure. H100 on a3-megagpu at $3.30/hr per GPU for large-scale training. TPU v5e at $2.50/hr for TensorFlow. G2 (L4) instances for cost-efficient inference.
