...
Voyage AI positions its models for high-stakes applications requiring maximum accuracy. The flagship Voyage-large-2-instruct model ranks 48th on MTEB and produces 1,536-dimensional vectors.
...
- **Superior clustering performance**: Ranks highest on clustering subtask benchmarks within the MTEB suite
- **Pricing transparency**: $0.08 per million tokens (moderate cost tier) with volume discounts available
...
| Voyage-large-2-instruct | Voyage AI | 1,536 | 48 | $0.08 | 50-70 | 12,000 | Instruction following |
...
- Voyage-large-2-instruct: 64.8% (best overall)
...
**Voyage AI** at $0.08 per million tokens carries premium pricing but justifies costs through superior MTEB performance. High-accuracy requirements or clustering workloads often recoup the cost premium through reduced document processing errors.
...
**Clustering and Recommendation**: Select Voyage-large-2-instruct. Superior clustering MTEB scores translate directly to recommendation quality. Teams investing in recommendation infrastructure recover the 4x cost premium through engagement improvements.
...
- Voyage-large-2-instruct: 100-token query costs $0.000008
...
**Infrastructure planning for on-premises embedding** requires capacity analysis. A single NVIDIA RTX 4090 ($1.40/hour on cloud) embeds approximately 30 million tokens daily. Storage costs for embeddings depend on dimensionality:
...
**Retrieval tasks** (most relevant for RAG systems) test finding relevant documents from large collections. Scores measure recall at position 100. Voyage-large-2-instruct leads at 64.8%, but text-embedding-3-small at 62.1% performs only 2.7% lower. In production, this difference rarely justifies upgrading unless retrieval accuracy directly impacts revenue. Teams building search products should conduct A/B testing: measure click-through rates on both models, quantify revenue impact, then calculate cost-benefit of premium model.

**Clustering tasks** evaluate grouping semantically similar documents. Voyage-large-2-instruct scores 47.6% versus text-embedding-3-small at 41.8%. The 5.8% gap is substantial. Teams building recommendation systems, content discovery, or topic clustering should prioritize Voyage if clustering accuracy drives engagement.
...
- Batch processing (overnight indexing, latency flexible): Route to Voyage-large-2-instruct
