...
| Claude Haiku 4.5 | $0.80 | $4.00 | 200K | 44 tok/s | Budget |
| Gemini 2.5 Pro | $1.25 | $10.00 | 1M | 40 tok/s | Mid-tier |
| Gemini 2.5 Flash | $0.075 | $0.30 | 1M | 50 tok/s | Ultra-budget |

**Data as of March 2026. All pricing in USD per million tokens.**

Claude Opus 4.6 costs 4x more per input token than Gemini 2.5 Pro. But Opus ships with 1M context by default; Gemini Pro starts there too. Haiku and Flash are the budget tiers: Haiku at $0.80/M input, Flash at $0.075/M input (essentially free for input, paid on output).
...
Gemini Flash's $0.075/M input is a pricing outlier. At scale (1B input tokens/month), input cost is under $75. Viable for cost-optimization of simple, high-volume tasks. But Flash's accuracy (27% on reasoning benchmarks) makes it unsuitable for anything but trivial classification.
...
**Gemini Flash:**
- 1M × 5K × $0.075/M + 1M × 500 × $0.30/M = $375 + $150 = $525

Gemini is 43x cheaper. At scale, cost difference is huge. Flash's 76% code accuracy doesn't matter for summarization (task is extracting key points, not correctness). Flash wins decisively here.
...
- Cost optimization on non-critical paths. $0.075/M input is negligible.

**Typical cost:** essentially free for input, minimal for output.
