...
| Multi-GPU NVLink | No | Via PCIe/NVLink | 900 GB/s NVLink | A100 |
...
**Bandwidth:** 2,039 GB/s (1,935 GB/s for 40GB variant).
...
The PCIe variant is the most common in boutique cloud providers because integration is simple: drop it in any standard server. The 80GB memory and 2TB/s bandwidth handle large model training better than RTX 4090's 24GB.

### A100 SXM4 (80GB)

**Memory:** 80GB HBM2e DRAM.
**Bandwidth:** 2.0 TB/s (SXM4 interconnect, higher than PCIe variant).
**TDP:** 400-700W (higher under load).
**NVLink:** 900 GB/s per GPU, scales to 57.6 TB/s aggregate across 256 GPUs.
...
**A100 SXM:** Eight A100s connected via NVLink achieve 900 GB/s per GPU for gradient synchronization. Training 70B models scales nearly linearly across GPUs (minimal communication overhead). This is why hyperscalers use A100 SXM for large model training.
