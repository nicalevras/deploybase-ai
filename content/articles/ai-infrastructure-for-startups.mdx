...
**Model Selection**: Most startups choose OpenAI GPT-4 ($0.03 input, $0.06 output) or GPT-5 ($1.25 input, $10 output depending on token volume). GPT-4 remains appropriate for most tasks. GPT-5 makes sense only if the application genuinely requires GPT-5's specific capabilities (long context windows, specific reasoning tasks). Roughly 70% of AI startups never need GPT-5.
...
Claude Sonnet 4.6 ($3 input, $15 output) costs 2.4-3x more but offers stronger instruction-following and safety characteristics. Evaluate only if the product depends on Claude's specific advantages.
...
At GPT-4 pricing ($0.015 average per 1K tokens): $1,237/month.

**2. What's the cost per token at API versus self-hosted?**

GPT-4: $0.015 per token average
GPT-5: $0.055 per token average
Gemini 2.5: $0.055 per token average
Self-hosted Llama 2 on H100: $0.0003 per token
...
API-First Cost (GPT-4 at $0.015/1K): 15B * $0.015/1K = $225K monthly
...
Interactive API cost: 5B * $0.015/1K = $75K monthly
...
API cost: 1.2B * $0.03/1K (average) = $36K monthly
...
Savings: $33K monthly by self-hosting

Justification: Save $33K monthly minus sunk infrastructure costs. If you have 1-2 engineers, their time cost ($200-300K annually) exceeds savings. Hire fractional DevOps person ($5-10K monthly) to manage infrastructure. Still save $23-28K monthly.
...
**For Anthropic Claude Users**: Claude Sonnet ($3 input/$15 output) covers most use cases, while Claude Opus ($15 input/$75 output) is the premium reasoning option. Both support long context windows enabling consolidation of multiple API calls. Consolidating 3 GPT-4 calls into 1 Claude call can save money despite higher per-token cost.
