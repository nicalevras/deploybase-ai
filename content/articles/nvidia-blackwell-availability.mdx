...
- Two B200 GPUs + custom NVLink fabric
- 208 TFLOPS FP8 combined (104 per GPU)
- 192GB total memory (96GB per GPU)
- Custom packaging reduces inter-GPU latency
- Designed for dense workloads requiring tight coupling
- Higher thermal envelope (600W per GPU)
- Delayed availability due to manufacturing complexity

The B200 offers 30% higher compute than H100 while matching memory capacity. GB200 enables scale-out architectures without host-to-device bottlenecks.
...
**LLM Inference:** B200 delivers 30% higher throughput than H100 at same latency, enabling cost reductions for inference APIs. A 70B parameter model serving 5k tokens/second on H100 scales to 6.5k tokens/second on B200 without additional hardware. This translates to 20% operational cost reduction for providers running inference at scale.
...
Current Blackwell pricing is not finalized across most providers:

**NVIDIA Cloud:** Estimated $12-15 per B200 GPU-hour based on H100 multiplier ($4/hour × 3-3.75x for Blackwell).

**Lambda:** Likely $10-12 per GPU-hour to remain competitive with existing H100 customers ($3.50/hour).

**CoreWeave:** Premium pricing at $15-20 per GPU-hour for guaranteed capacity.

**Spot markets:** $8-12 per GPU-hour when available (30-40% spot discount typical).

Pricing will decline 30-50% by late 2025 as production ramps and competition increases. Early adopters pay premium rates for marginal performance gains.
