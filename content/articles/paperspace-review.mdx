...
A100 80GB: $0.40-0.50/hr. Good for large models.

H100: $1.98/hr when available. Spotty across regions.
...
Paperspace's pricing model combines hourly compute costs with optional storage and bandwidth charges. Standard on-demand pricing for A100 80GB ranges from $0.40 to $0.50 per hour depending on region, lower than single-GPU pricing but higher than multi-GPU cluster aggregates at competitor providers.
...
A typical workflow using an A100 notebook for experimentation (32 hours per month at $0.45/hr = $14.40), storage for 100GB datasets ($10/month), and periodic weight downloads (roughly $5/month in bandwidth) costs approximately $29.40 monthly for light experimentation.

This contrasts with [RunPod](/articles/runpod-gpu-pricing) A100 pricing at $1.52/hr for 80GB instances, which becomes more economical for intensive training jobs exceeding 20 hours monthly but less cost-effective for light experimentation.
