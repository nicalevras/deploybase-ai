...
Claude offers three tiers. Sonnet 3.5 is the price-to-performance sweet spot at $3 per million input tokens and $15 per million output tokens. Haiku 3.5, the fastest, costs $0.80 input and $4 output per M tokens. Opus, the flagship, runs $15 input and $75 output per M tokens. Full details: [Anthropic pricing guide](/llms/Anthropic).

A typical application sending 10M tokens monthly through Sonnet costs roughly $120 (1:5 input-to-output ratio). Same workload on Opus: $600.

OpenAI's pricing varies by model. GPT-4 Turbo costs $10/$30 per M tokens - expensive relative to Sonnet. GPT-4o, the flagship, costs $5/$15 per M tokens (matches Sonnet on output, higher on input). GPT-4 mini costs $0.15/$0.60 per M tokens.

Same 10M token workload: GPT-4o hits $130, GPT-4 mini costs $6.30.

This creates tiers. Mini models (Haiku, GPT-4 mini) work for high-volume, low-complexity tasks. Standard models (Sonnet, GPT-4o) balance cost and capability. Flagship models (Opus, GPT-4 Turbo) for when quality justifies the premium.

Claude's input pricing undercuts OpenAI, especially for long contexts or frequent RAG calls. OpenAI's output pricing matches Claude mid-tier, but mini output tokens cost 25% less.
...
Claude 3.5 Sonnet: 200K token context (roughly 140K words). The entire Harry Potter series is ~1.1M words, so Sonnet can't fit that. A typical novel fits fine.

Haiku: 200K context too. Opus: 200K. All three models match here.

OpenAI's GPT-4o: 128K tokens (~90K words). GPT-4 Turbo: 128K. GPT-4 mini: 128K.

Claude's 200K advantage matters. A 10-20 document retrieval system costs 40% fewer input tokens on Claude. Code analysis systems loading entire repositories cost 55% fewer input tokens.
...
**Choose Claude if:** Input costs matter (long contexts, frequent RAG), need 200K context windows, fine-tuning ROI is high, batch processing is in scope, or prompt caching fits usage patterns.

**Choose OpenAI if:** High request volume with small requests, need top-tier reasoning (GPT-4 Turbo), require published SLAs, or team knows OpenAI already.
...
- Claude Sonnet: 100 * (0.5 * 3 + 0.2 * 15) = $30/month
- GPT-4o: 100 * (0.5 * 5 + 0.2 * 15) = $35/month
- GPT-4 mini: 100 * (0.5 * 0.15 + 0.2 * 0.60) = $19.50/month

GPT-4 mini wins, though quality trade-offs should be considered. If capability matters more, Claude and GPT-4o are similar.

**Scenario 2: High-volume batch document analysis (50M tokens/month, 5:1 input-to-output ratio)**
- Claude Sonnet (25M input, 25M output): 25 * 3 + 25 * 15 = $450
- Claude with 50% batch discount: $225
- GPT-4o: 25 * 5 + 25 * 15 = $500
- GPT-4 mini: 25 * 0.15 + 25 * 0.60 = $19.75

Claude with batch processing is least expensive. If capability requirements favor a larger model, the cost advantage grows.

**Scenario 3: Long-context analysis (200K tokens per request, 10 requests/month)**
- Claude Sonnet: 10 * (200 * 3 + 50 * 15) = $13,500
- GPT-4o: 10 * (128 * 5 + 50 * 15) = $13,400

Here context length helps Claude: fewer tokens needed to accomplish the same work. With prompt caching enabled, Claude costs $1,350.
...
**Startup MVP:** Start with GPT-4 mini for cost. Switch to Claude Sonnet as traffic grows and token patterns become clear.
...
Claude's input price ($3/1M vs $5/1M) plus batch discount = 30-40% savings. Winner: Claude.

**Cost-Sensitive Prototyping:**
GPT-4 mini at $0.15/$0.60 wins. Lower quality, but feedback is fast and cheap. Winner: GPT-4 mini.
...
200,000 tokens, sufficient for approximately 140,000 words of input.
