...
The A100 40GB uses HBM2e (High Bandwidth Memory 2e) with a 5,120-bit bus, delivering 1.6 TB/s peak bandwidth. The 80GB model uses the same HBM2e technology but doubles the stacks, maintaining the same 1.6 TB/s per stack approach. Here's what that means in practice:
...
Both SKUs contain 6,912 CUDA cores organized into 432 SMs (Streaming Multiprocessors). Each SM has 64 FP32 cores, 64 INT32 cores, 32 FP64 cores, 4 Tensor cores for sparsity, and 2 NVLink 3.0 ports (for 8-GPU configurations). Peak FP32 performance: 312 TFLOPs for either variant. Peak Tensor core throughput (FP8): 1,248 TFLOP on both.

The compute ceiling is identical. Memory is what separates them.

### Clock Speeds

Both ship at the same boost clock (1.41 GHz nominal, up to 1.87 GHz under thermal headroom). Both support the same Tensor core data types: TF32, BF16, FP16, and FP8. Power consumption: 250W TDP baseline, though multi-GPU configurations can draw significantly more.
...
Both A100 SKUs achieve identical 1.6 TB/s per-stack bandwidth. However, the practical throughput ceiling depends on cache efficiency and kernel design. In PyTorch training, realistic memory bandwidth utilization hovers at 30-60% of peak due to synchronization overhead and kernel launch latency.
...
Both variants carry identical power envelopes (250W TDP). The 80GB doesn't consume more power. This is an advantage: you gain capacity with zero thermal overhead.
...
NVLink 3.0 offers 900 GB/s per link. An A100 with two NVLink ports sustains 1.8 TB/s GPU-to-GPU within a node. This is faster than the 1.6 TB/s to HBM, making intra-node collective operations (all-reduce for distributed training) faster than fetching from memory.
...
- [Lambda Labs GPU Pricing](https://www.lambdalabs.com/)
