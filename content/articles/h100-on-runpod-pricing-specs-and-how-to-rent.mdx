...
H100 PCIe: 80GB HBM3 memory, 700W power consumption, PCIe 5.0 interface
H100 SXM: 80GB HBM3 memory, 700W power consumption, NVLink interconnect capable

RunPod offers both variants at different price points. SXM variant at $2.69/GPU-second provides better multi-GPU scalability.
...
H100 PCIe: $1.99 per GPU-second ($1.99 per second of GPU time)
H100 SXM: $2.69 per GPU-second

RunPod's per-second billing is unique among providers. Billing increments at per-second granularity, so a 30-second job costs roughly $0.06-0.08.

For monthly estimates:
- H100 PCIe at 24/7 operation: $1.99 × 86,400 × 30 = $5,183/month
- H100 SXM at 24/7 operation: $2.69 × 86,400 × 30 = $6,998/month

For intermittent usage:
- 1 hour usage: H100 PCIe = $7.16, H100 SXM = $9.68
- 8 hours daily usage: H100 PCIe = $57/day = $1,710/month, H100 SXM = $77/day = $2,310/month

RunPod's pricing advantage emerges with sporadic or batch workloads where per-second billing avoids waste.
...
Compare:
- Pod: $7.16/hour × 24 hours × 30 days = $5,155/month for continuous operation
- Serverless: Same H100 at $0.30/request × 1,000 requests = $300/month (if generating 1,000 requests monthly)

Pod breaks even at ~20,000 requests monthly (assuming 10 seconds per request).
...
Use H100 SXM ($2.69/second) only for multi-GPU distributed training. Single-GPU workloads should use H100 PCIe ($1.99/second) to save 26%.
...
RunPod is cheaper ($1.99-2.69 per second vs Lambda's $2.86 per hour). Use RunPod for sporadic or batch workloads. Use Lambda for sustained workloads requiring 99% SLA and responsive support.
...
Use H200 at $3.59/second (141GB) on RunPod, or multiple H100s with distributed inference. Neither option offers more than 80GB per H100 device.
...
- Hardware: H100 SXM at $2.69/second
- Monthly cost: 8 × 5 × 4.3 × 3,600 × $2.69 = $1,967/month
- Break-even vs ownership: 8+ months payback with $10,000 GPU purchase
...
- Monthly cost: 24 × 30 × 3,600 × $1.99 = $5,155/month
...
- Hardware: H100 PCIe at $1.99/second
- Monthly cost: 83 × 30 × 3,600 × $1.99 = $17,850/month
