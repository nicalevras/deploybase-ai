...
| H100 SXM | 80GB | $2.49 | $1,817.70 | Large model training, multi-GPU clusters |
...
H100 SXM at $2.49/hr is the distributed training workhorse. Same 80GB VRAM, but 3.35 TB/s memory bandwidth and NVLink 4.0 interconnect (900 GB/s per GPU). Eight-GPU clusters reach 57.6 TB/s aggregate bandwidth. Essential for training 70B+ models or large model parallelism. Priced below H100 PCIe ($2.86/hr), the SXM form factor is excellent value for multi-GPU deployments.
...
| Configuration | VRAM Total | Price/hr | Price/GPU |
|---------------|------|----------|----------|
| 1x H100 SXM | 80GB | $2.49 | $2.49 |
| 8x H100 SXM | 640GB | $19.92 | $2.49 |

Eight H100 SXMs cost $19.92/hr total, or $2.49 per GPU. The per-GPU rate is identical to a single H100 SXM rental. Lambda prices H100 SXM linearly with no bulk premium or discount at this tier.
...
- H100 SXM single: $2.49/hr = $1,818/month on-demand → ~$1,273/month reserved
- H100 SXM 8x cluster: $19.92/hr = $14,542/month on-demand → ~$10,179/month reserved
...
Lambda 8x H100 SXM cluster: $19.92/hr × 168 hours = **$3,346.56**
...
H100 SXM: $2.49/hr × 4 hrs/day × 30 days = **$298.80/month**
...
Lambda 8x H100 SXM cluster: $19.92/hr × 216 hours (9 days) = **$4,302.72**
...
|----------|------|-----------|----------|-------|
| Lambda | $1.48 | $2.86 | $2.49 | Consistent pricing, strong support |
| RunPod | $1.19 | $1.99 | $2.69 | 20-40% cheaper across most tiers |
| Vast.AI | [Variable] | [Variable] | [Variable] | Marketplace model, spot rates volatile |

RunPod undercuts Lambda by 20-40% across most tiers. For example:
...
- H100 SXM: Lambda $2.49 vs RunPod $2.69 = RunPod is $0.20/hr more expensive. Lambda wins on H100 SXM pricing.

At 8x H100 scale:
- Lambda 8x: $19.92/hr
- RunPod 8x: $21.52/hr (8 × $2.69)

RunPod's per-GPU rate on multi-GPU clusters approaches Lambda's cluster rates.
...
If using SXM for a single-GPU workload, cost is $2.49/hr — actually cheaper than PCIe ($2.86/hr), making SXM the better value for all H100 workloads on Lambda.

**Multi-GPU workload (training large model):**
- Use H100 SXM for NVLink
- H100 SXM 8x costs $19.92/hr = $2.49 per GPU
- Running 8x PCIe separately is $2.86 × 8 = $22.88/hr
- SXM cluster saves 13% ($2.96/hr) and has better bandwidth
...
Example: if Lambda offers 50% discount for reservations, a 8x H100 SXM cluster at $21.68/hr becomes $10.84/hr reserved. Over a month of continuous training, savings approach $8,000.
...
**Why is H100 SXM cheaper than PCIe on Lambda?**
H100 SXM at $2.49/hr is priced below H100 PCIe at $2.86/hr on Lambda. Despite having superior specs (3.35 TB/s bandwidth vs 2.0 TB/s, NVLink 4.0 interconnect), Lambda prices SXM more aggressively. For multi-GPU training, H100 SXM is both faster and cheaper per GPU — the clear choice.
