...
Modal operates on a per-request model with pay-as-you-go GPU allocation. The platform charges based on GPU hours consumed during function execution, plus egress costs. Modal's abstraction layer handles container management, so users don't manage infrastructure directly.
