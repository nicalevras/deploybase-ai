...
|-------|--------|---------|----------|---------|---------------------|
| **Llama 4 Maverick** | 400B total (17B active MoE) | 1M | General reasoning, multimodal | Open | $1,200-$2,400 (8xH100) |
...
Performance: Competitive on many benchmarks. Scores 78% on MMLU (general knowledge), 84% on GSM8K (mathematical reasoning). The efficient MoE design means inference cost is similar to a 17B dense model despite broader knowledge capacity.

Architecture: Sparse MoE with 16 expert modules. Router selects active experts per token. Only 17B parameters activate per inference step, making it faster and cheaper than its 109B total parameter count suggests.
...
Performance: 74% MMLU, 84% GSM8K, 73% HumanEval. Capabilities are lower than large models, but dense (high quality relative to size). Punches above its weight for the 12B class.
