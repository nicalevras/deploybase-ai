...
GB200 is NVIDIA's Grace Blackwell superchip: one Grace CPU (72 ARM Neoverse V2 cores, 480GB LPDDR5x) paired with one B200 GPU, connected via NVLink-C2C at 900 GB/s. Two GB200 superchips form the GB200 NVL2 module. Each B200 has 192GB HBM3e memory at ~8 TB/s bandwidth. Built for scale.
...
8x GB200 cluster: $68.80/hour ($8.60 per GPU). Fixed pricing. 100GB Ethernet included.

For context, [8x H100 costs $49.24/hour](/articles/coreweave-gpu-pricing) on CoreWeave. GB200 is 40% more expensive. The memory gain (192GB vs 80GB) and better inter-GPU bandwidth justify it for mega-scale inference. For most workloads, H100s or H200s are fine.
