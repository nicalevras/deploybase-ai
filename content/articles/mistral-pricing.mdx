...
description: >-
  Mistral AI API pricing 2026. Large $12/M combined, Medium $1.08/M, Small $0.40/M
...
Mistral pricing covers three API tiers as of March 2026: Mistral Small (lightweight inference), Mistral Medium (balanced), and Mistral Large (frontier capability). Pricing ranges from $0.10 per million input tokens (Small) to $9.00 per million output tokens (Large). All models are accessed via API (mistral.AI) with OpenAI-compatible endpoints. Mistral also publishes open-source weights for Small and Medium, allowing teams to self-host and avoid per-token API costs entirely. European data residency compliance and competitive pricing have driven adoption among European companies and cost-conscious teams.
...
| **Large** | $3.00 | $9.00 | $12.00 | 128K | Complex reasoning, code, long context |
...
Large is a 123B parameter model. 128K context window. $3.00/M input, $9.00/M output = $12.00 per million combined tokens. Frontier capability.
...
Capability: comparable to OpenAI o1-mini on reasoning and coding benchmarks. Weaker than o1 (OpenAI's reasoning-optimized model) but faster. Competitive with Anthropic Claude Opus on multi-turn chat but cheaper per output token ($9.00 vs $25 on Claude Opus).
...
Monthly cost (100M input + 50M output tokens): (100M × $3.00 + 50M × $9.00) / 1M = $750. Production-level workload. Comparable to Claude Sonnet ($18/M × 150M tokens = $2,700/month, 3.6x more expensive than Mistral Large).
...
API cost for same tokens at $12.00/M combined: 1.04B × $12.00 = $12,480/month.

Self-hosting 123B saves $12,480 - $7,855 = $4,625/month. At this scale self-hosting becomes cost-effective, unlike smaller models where API is cheaper.
...
**Mistral Large (API):**
- Total: 22.5M × $12.00 = $270/month

For IDE use case, volume is low (<100M tokens/month). API is cost-effective. Use Mistral Medium ($24/month) for better reasoning than Small on complex code problems. Upgrade to Large ($270/month) if developer team is working on reasoning-heavy problems (algorithm design, architectural decisions).
...
| **Mistral Large** | $3.00 | $9.00 | $12.00 | Complex reasoning |
...
Mistral Small is 47% cheaper than GPT-4o Mini ($0.40 vs $0.75 combined). Mistral Large ($12.00 combined) is comparable to OpenAI GPT-4 Turbo ($10.00) on price. Mistral competes aggressively on price across all tiers.

DeepSeek-V3 ($0.42/M combined) is significantly cheaper than Mistral Small ($0.40/M) and offers equivalent quality. DeepSeek Large equivalent would be cheaper than Mistral Large ($12.00 combined).
...
Small (7B): lightweight, $0.40/M tokens combined. Medium (30-40B): balanced, $1.08/M tokens combined. Large (123B): frontier reasoning, $3.00/M input, $9.00/M output. Use Small for classification/embeddings. Use Medium for chat/RAG. Use Large for complex reasoning/code.
...
Yes. Mistral Small ($0.40/M combined) is 47% cheaper than GPT-4o Mini ($0.75/M). Mistral Large ($12/M combined) is comparable to GPT-4 Turbo ($10/M). Capability gaps are small (<10% on reasoning benchmarks).
...
Mistral Large (MATH benchmark): 67% accuracy. Claude Opus: 68%. Gap is 1%, negligible. Mistral Large input ($3.00/M) is significantly cheaper than Claude Opus ($5.00/M input, $25.00/M output).
