...
Llama 4 Scout represents Meta's efficient variant within the Llama 4 family, optimized for inference rather than maximum capabilities. With 17B active parameters (49B total with MoE architecture), Scout requires approximately 34GB of VRAM for FP16 weight loading. This fits within a single [H100 GPU ($2.49/hour on Lambda)](/articles/lambda-labs-gpu-pricing), enabling cost-effective experimentation and prototyping.
...
Full fine-tuning updates all model weights, requiring approximately 150GB of memory for training a 17B model with gradient accumulation and optimizer states. This necessitates [A100 40GB or H100](/articles/nvidia-h100-price) pairing (or multi-GPU setups), driving training cost to $500-1,500 per training job. For teams experimenting iteratively, these costs compound rapidly across multiple training runs.
...
Even relatively expensive setups (using H100 at $2.49/hour) cost less than $100 for complete fine-tuning on datasets below 20,000 examples.
...
- Developers have 1,000+ domain examples already available or easily collectible
...
- Developers lack domain-specific training examples (collecting them costs more than benefit)
