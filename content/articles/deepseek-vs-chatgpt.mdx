...
|-------|------------|-------------|---------|-----------|------|
| DeepSeek V3.1 | $0.27 | $1.10 | 256K | Medium | Budget |
| DeepSeek R1 | $0.55 | $2.19 | 256K | Strong (CoT) | Mid |
...
DeepSeek uses Mixture of Experts (MoE) architecture. 37 billion total parameters, but only 5 billion active per token (sparse routing). This is key: only a fraction of parameters are computed for each token, reducing compute and latency.
...
- Medium context (256K, smaller than GPT-5's 272K)
...
DeepSeek: 256K context. ChatGPT: 272K context (GPT-5.1: 400K). Difference is small, but for huge documents (>200K tokens), GPT-5.1 is better.
