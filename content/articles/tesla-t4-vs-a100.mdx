...
The A100 provides 80GB of HBM2e memory at 2.0 TB/s bandwidth, approximately 6.7x the capacity and 6.7x the bandwidth of T4. This enables serving models 20+ billion parameters with substantial batch processing overhead.
...
T4 instances cost approximately $0.20-$0.76 per hour depending on cloud provider and commitment level. AWS charges $0.35/hour for on-demand T4 instances. Google Cloud provides T4 instances at $0.25/hour with sustained-use discounts reducing effective cost to $0.18/hour.

A100 instances cost approximately $1.19-$1.48 per hour depending on provider and commitment level. Lambda Labs charges $1.19/hour for A100 instances. Paperspace prices A100 at $1.35/hour.
...
**T4 approach**: 1M tokens / 40 tokens/sec = 25K seconds per day = 7 hours. At $0.35/hour, daily cost = $2.45. Annual cost = $894.

**A100 approach**: 1M tokens / 300 tokens/sec = 3.3K seconds per day = 0.92 hours. At $1.35/hour, daily cost = $1.24. Annual cost = $453.

The A100 provides 2x better cost-per-token efficiency despite higher absolute hourly cost, demonstrating that raw GPU cost does not determine inference economics.

Scaling this analysis to monthly: 30M tokens monthly costs $26.95 on T4, $37.20 on A100. The T4 becomes more cost-effective for modest monthly volumes. But at 300M tokens monthly ($269.50 on T4, $372 on A100), the A100 shows slight advantage. Above 600M tokens monthly, A100 cost advantage compounds substantially.
...
For teams serving 7+ models concurrently, A100 with MIG provides superior cost-effectiveness compared to managing multiple T4s. A team serving 7 classification models could use a single A100 with MIG ($1.35/hr) instead of 7 T4s ($2.45/hr), plus simplified operations.
...
A: Rent, almost always. [Lambda Labs offers A100 at $1.19/hr](/articles/lambda-labs-gpu-pricing), [RunPod T4 at lower rates](/articles/runpod-gpu-pricing). Renting provides flexibility without capital risk.
