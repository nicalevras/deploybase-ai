...
1. **Groq (LPU-based)**: 8,000-10,000 tokens/second (custom silicon)
2. **Fireworks (GPU, optimized inference)**: 2,000-4,000 tokens/second ([H100](/articles/nvidia-h100-price)s optimized)
3. **Cerebras (custom silicon)**: 2,000-6,000 tokens/second (alternative custom hardware)
4. **Together (GPU, distributed)**: 1,500-3,000 tokens/second (distributed GPU clusters)
5. **Standard GPU APIs** (Replicate, Baseten): 500-1,500 tokens/second (commodity GPU instances)
6. **Major LLM APIs** (OpenAI, Anthropic): 100-500 tokens/second (general-purpose APIs, rate-limited)

Groq's 10,000 token/second speed is extraordinary. To put this in perspective, a typical response is 100-200 tokens. At Groq speed, responses complete in 10-30ms. At OpenAI speed, the same response takes 100-400ms. The difference between "feels responsive" and "feels slow."

**Real-world example**: A chat application with 50k daily active users, average 2 messages per user daily = 100k messages.
- Groq at $0.006 per request average = $600/day = $18,000/month
- OpenAI at $0.02 per request average = $2,000/day = $60,000/month
- Fireworks at $0.0006 per request average = $60/day = $1,800/month

Groq costs 10x more than Fireworks but provides better user experience. OpenAI costs 30x more than Fireworks.
...
**Pricing**: Groq charges $5 per 1M input tokens, $15 per 1M output tokens. For a typical 50-token response with 500-token context:
- Input cost: 500 tokens × $5/1M = $0.0025
- Output cost: 50 tokens × $15/1M = $0.00075
- Total: ~$0.003 per request

For 1M requests monthly: $3,000.

**Advantages**:
- Extreme speed (2-10x faster than competitors)
- Consistent latency (sub-100ms even under load)
- Good pricing ($5/$15 matches mid-tier APIs)
- Strong model support (Llama, Mixtral, and others)
...
**Pricing**: Cerebras pricing is opaque (contact sales), estimated $3-10 per 1M input tokens. Likely similar or slightly lower than Groq.
...
|-----|-----------|-----------|-------------------|------------------------|-----------|
| Groq | $5 | $15 | 8,000 | $0.003 | 1st (fastest) |
...
Groq's limited model variety is significant. As of early 2026, Groq supports:
- Llama 3 (8B, 70B)
- Mixtral 8x7B
- Mixtral 8x22B
