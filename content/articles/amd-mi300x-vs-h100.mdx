...
| **Peak FP32** | 51 TFLOPS | 67 TFLOPS | H100 (31% faster) |
| **Peak FP8 Tensor** | 12,300 TFLOPS | 5,825 TFLOPS | MI300X (2.1x) |
| **TF32 Tensor** | 1,600 TFLOPS | 989 TFLOPS | MI300X (62% faster) |
| **AI Accelerators (per GPU)** | 3 (SPUs) | 2 (TCs) | MI300X (more units) |
| **Software Stack** | ROCm | CUDA | H100 (maturity) |
| **Cloud Price/hr** | $4.50-$5.50 | $2.49-$2.86 | H100 (cheaper) |
| **Price per GB Memory** | $0.12/GB | $0.50/GB | MI300X (4x cheaper) |
...
MI300X:
- 288 compute units
- 55,296 stream processors
- Peak single precision (FP32): 51 TFLOPS
...
- MI300X: 12,300 TFLOPS
- H100: 5,825 TFLOPS
- Advantage: MI300X (2.1x)

For quantized inference, MI300X is significantly faster. Critical for serving large models with minimal accuracy loss.

**TF32 Tensor (training):**
- MI300X: 1,600 TFLOPS
- H100: 989 TFLOPS
- Advantage: MI300X (62% faster on paper; real-world gap is smaller due to ROCm maturity)
...
- Cost: $4.99/hr (estimated cloud rate)

**Cost-per-token:**
- H100: $1.99/hr at 850 tok/s = $2.34 per million tokens
- MI300X: $4.99/hr at 1,050 tok/s = $4.75 per million tokens

H100 is cheaper per token on inference, despite MI300X's FP8 advantage. The price premium outweighs throughput gains.
...
| Lambda | H100 SXM | 80GB | $2.49 |
| Crusoe Energy | MI300X | 192GB | $4.50 |
| CoreWeave | MI300X | 192GB | $5.50 |

MI300X is 1.5-2.0x more expensive per GPU-hour. But MI300X has 2.4x more memory, so price-per-GB is lower.

### Price per GB Memory

- H100: $2.49-$2.86 per GPU-hour for 80GB = $31-36 per GB-hour
- MI300X: $4.50-$5.50 per GPU-hour for 192GB = $23-29 per GB-hour

MI300X is cheaper per GB of memory. For workloads where memory is the constraint (like 200B model training), MI300X is economical.
...
- Monthly savings: $15,764 (H100 cluster) - $3,642 (MI300X) = $12,122
- Annual savings: ~$145k
