...
Pro ($1.50 input, $6 output per 1M) for reasoning.

Flash ($0.075 input, $0.30 output) for speed.
...
|-------|-----------|----------------|-----------------|----------|
| **Free Tier** | Free (50 req/min) | Free (50 req/min) | None | Prototyping |
| **Gemini 2.5 Pro** | $1.50 | $6.00 | 90% discount | Complex reasoning |
| **Gemini 2.5 Flash** | $0.075 | $0.30 | 90% discount | Speed-focused, cost-sensitive |
...
Pro is the flagship model. Handles reasoning, code generation, multimodal analysis (text, image, video, audio). Supports 200K context window.

**Per-million-token costs (standard):**
- Prompt tokens: $1.50/M (0.15¢ per 100 tokens)
- Completion tokens: $6.00/M (0.60¢ per 100 tokens)
- Batch processing: same rates, asynchronous

Completion tokens cost 4x prompt tokens, creating incentive to optimize output length.

**Monthly cost estimate (example):**

A team processing 100M prompt tokens + 25M completion tokens per month:
- Prompts: 100M × $1.50/M = $150
- Completions: 25M × $6.00/M = $150
- Total: $300/month

**Tier-down strategy:** If 80% of requests could use Flash (cheaper model), mix:
- 80M prompt tokens on Flash: 80M × $0.075/M = $6
- 20M prompt tokens on Pro: 20M × $1.50/M = $30
- 5M completion tokens on Flash: 5M × $0.30/M = $1.50
- 20M completion tokens on Pro: 20M × $6.00/M = $120
- Total: ~$157 (vs $300, 48% savings)

### Context Caching with Pro

With context caching, cached prompt tokens drop to $0.15/M (10% of standard rate). For agents with system prompts, knowledge bases, or few-shot examples exceeding 10K tokens, caching saves 10-40% on total monthly costs.

**Example: 50K-token cached system prompt + knowledge base**
- Cached tokens: 50K × $0.15/M = $0.0075
- Uncached tokens: 10K × $1.50/M = $0.015
- Total per request: $0.0225 (vs $0.090 without caching, 75% savings)
...
- Total input: 450K tokens × $1.50/M = $0.68
- Total output: 100K tokens × $6.00/M = $0.60
- Total: $1.28 for 1,000 images = $0.00128 per image
...
Flash is the lightweight model. Optimized for speed and cost, not reasoning. Typical latency: 200-400ms (vs Pro's 500-800ms). Supports same 200K context window as Pro.

**Per-million-token costs:**
- Prompt tokens: $0.075/M (0.0075¢ per 100 tokens)
- Completion tokens: $0.30/M (0.03¢ per 100 tokens)
- 20x cheaper than Pro on both prompts and completions.

**Monthly cost comparison (same 100M + 25M volume as Pro):**
- Prompts: 100M × $0.075/M = $7.50
- Completions: 25M × $0.30/M = $7.50
- Total: $15/month (vs $300 for Pro, 95% savings)
...
Flash with caching: prompt cost drops to $0.0075/M (90% discount on cached tokens). For high-volume workloads with repeated context:

**Example: Support ticket classification (cached response templates)**
- Cached templates (5K tokens): 5K × $0.0075/M = $0.0000375
- Uncached ticket text (500 tokens): 500 × $0.075/M = $0.0000375
- Output (classification + suggestion): 100 tokens × $0.30/M = $0.00003
- Total per ticket: $0.000076
- For 100K tickets/month: $7.60/month
...
**How it works:**
1. Upload a system prompt or few-shot context (e.g., 8K-token jailbreak examples)
2. Mark with `cache_control` = "ephemeral" in the API request
3. Google caches the embedding for 5 minutes (free tier) to 24 hours (paid)
...
- Example: 10K cached tokens + 500 new tokens in Pro
  - Cached: 10K × $1.50/M × 0.10 = $0.015
  - Uncached: 500 × $1.50/M = $0.00075
  - Total: $0.016 (vs. $0.015 without caching, marginal savings on small prompts)
...
- Prompts: 30K × $0.075/M (using Flash) = $0.002
- Completions: 15K × $0.30/M = $0.005
- Total: $0.007/day, ~$2.10/month

**Daily cost (with 5-minute cache):**
- Cached system prompt: 500 × $0.0075/M = $0.000004
- Uncached email text: 30K × $0.075/M = $0.002
- Completions: 15K × $0.30/M = $0.005
- Total: $0.007/day (negligible savings at this scale)
...
- Prompts: 10M × $0.075/M = $750
- Completions: 1M × $0.30/M = $300
- Total: $1,050/day, ~$31.5K/month

**Daily cost (Flash with caching):**
- Cached system prompt: 300 × $0.0075/M × 50K requests = $11.25
- Uncached comment text: 10M × $0.075/M = $750
- Completions: 1M × $0.30/M = $300
- Total: ~$1,061/day (caching adds negligible savings due to unique comment text)
...
- Prompts: 40.7K × $1.50/M = $0.061
- Completion: 500 tokens × $6.00/M = $0.003
- Total: $0.064 per research synthesis

**Cost per request (Pro with 24-hour cache):**
- Cached system prompt: 500 × $0.15/M = $0.00008
- Cached source documents (reused across requests): 40K × $0.15/M = $0.006
- Uncached user query: 200 × $1.50/M = $0.0003
- Completion: 500 × $6.00/M = $0.003
- Total: ~$0.009 per research synthesis (86% savings if sources are reused)
...
|----------|-----------|----------------|--------|----------------|
| **Gemini Pro** | $1.50 | $6.00 | 90% off | Reasoning, multimodal |
| **OpenAI GPT-4o** | $2.50 | $10.00 | None | Production, safety |
| **Claude Opus** | $5.00 | $25.00 | 90% off | Complex analysis |
| **Gemini Flash** | $0.075 | $0.30 | 90% off | Cost-first, speed-first |
| **OpenAI GPT-4o Mini** | $0.15 | $0.60 | None | Fast, affordable |

Gemini Flash undercuts GPT-4o Mini by 2x. Gemini Pro is 40% cheaper than GPT-4o while matching capability. Caching is Gemini's structural advantage; competitors have no equivalent discount mechanism.
...
- Without caching: 10K × 2K × $1.50/M = $30
- With caching: 10K × (2K × $1.50/M × 0.10 + overhead) = ~$3-5
- Savings: ~$25-27
...
- Query: 50 tokens
- Response: 200 tokens
- Cost per receipt: (550 × $1.50 + 200 × $6.00) / 1M = $0.0013
...
Gemini is 36x cheaper for this workload because images are tokenized (not separately metered).
...
Version 2.5 is 10-15% faster and has better reasoning on code. Token pricing is identical ($0.075/$0.30). If using free tier, you get 1.5 Flash (older). Paid users should default to 2.5.
...
**Example: Email classification with system prompt**
- System prompt (cached): 1K tokens × $0.0075/M = $0.0000075
- Email content: 200 tokens × $0.075/M = $0.000015
- Classification output: 20 tokens × $0.30/M = $0.000006
- Total per email: $0.0000285
- For 100K emails/month: $2.85/month
...
|-------|-----------|-----------|----------------|----------|
| **Gemini 2.5 Flash** | $0.075 | $0.30 | 4:1 | Speed, cost |
| **Gemini 2.5 Pro** | $1.50 | $6.00 | 4:1 | Reasoning |
...
**Gemini Flash is 2x cheaper than GPT-4o Mini and 13x cheaper than Claude Opus.** For cost-optimized inference at scale, Flash dominates.
...
**Gemini Flash:**
- Input: 500M × $0.075/M = $37.50
- Output: 500M × $0.30/M = $150
- Total: $187.50/month
...
Gemini Flash is **2x cheaper than GPT-4o Mini** and **16x cheaper than Claude Haiku** at this scale. Savings of $187.50/month compound to $2,250/year.
...
**Cost:** $1.50-6.00 per request (depending on volume).
...
**Cost:** ~$0.075-0.30 per request on average (30-40% Flash, 10% Pro escalation, 60% Flash-only).
...
**Cost:** Same per-token ($1.50/$6.00 for Pro) but amortized overhead.
