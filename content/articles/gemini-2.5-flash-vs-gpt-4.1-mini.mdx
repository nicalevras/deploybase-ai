...
Gemini 2.5 Flash vs GPT-4.1 Mini is the focus of this guide. Gemini 2.5 Flash: $0.075 input, $0.30 output. Cheap on input.

GPT-4.1 Mini: $0.40 input, $1.60 output. Cheaper on output. 75 tokens/sec.

Both are budget models. Flash if developers care about input cost. Mini if developers care about speed.
...
|-----------|---|---|---|
| **Input price $/M** | $0.075 | $0.40 | Gemini |
| **Output price $/M** | $0.30 | $1.60 | OpenAI |
...
| **Cost for 10M input + 5M output** | $2.25 | $12.00 | Gemini |
...
Flash is Google's speed-optimized model. Released March 2026 as the successor to Gemini 2.0 Flash. Context window: 1 million tokens (equivalent to 750K words or 15K lines of code). Input costs $0.075/M, output $0.30/M.
...
No free tier. Requires API key and billing. The $0.40 input price is 5.3x more than Gemini Flash input ($0.075).
...
|----------|---|---|---|
| 1M input tokens | $0.075 | $0.40 | Gemini |
| 1M output tokens | $0.30 | $1.60 | Gemini |
| 10M in + 5M out | $2.25 | $12.00 | Gemini |
| 100M in + 50M out | $22.50 | $120.00 | Gemini |
| 1B in + 500M out | $225 | $1,200 | Gemini |

Gemini Flash is cheaper overall because both input ($0.075/M vs $0.40/M) and output ($0.30/M vs $1.60/M) are lower. Flash wins decisively on cost.

If the application is input-heavy (e.g., document analysis where teams feed 100K tokens and get back 500 tokens), Gemini Flash edges ahead.
...
- **Gemini 2.5 Flash**: (30B × $0.075 + 3B × $0.30) / 1M = $2,250 + $900 = **$3,150/month**
- **GPT-4.1 Mini**: (30B × $0.40 + 3B × $1.60) / 1M = $12,000 + $4,800 = **$16,800/month**

Gemini is 5x cheaper at this scale. Flash wins decisively on both input and output token costs.

For high-output applications (code generation, creative writing):

- **Gemini 2.5 Flash**: (1B in + 10B out) = $75 + $3,000 = **$3,075/month**
- **GPT-4.1 Mini**: (1B in + 10B out) = $400 + $16,000 = **$16,400/month**

Gemini wins on heavy output workloads too.

For input-heavy applications (classification, extraction):

- **Gemini 2.5 Flash**: (100B in + 500M out) = $7,500 + $150 = **$7,650/month**
- **GPT-4.1 Mini**: (100B in + 500M out) = $40,000 + $800 = **$40,800/month**

Gemini dominates on heavy input workloads.
...
**Document analysis at scale**. 1M context, $0.075 input cost. Analyze long PDFs, codebases, or regulatory filings in a single API call. No token overhead from splitting documents.
...
**Budget input-heavy applications**. Document classification, log analysis, batch extraction across millions of documents. $0.075 input crushes $0.40.
...
Gemini Flash is cheaper on both input ($0.075 vs $0.40) and output ($0.30 vs $1.60). Flash wins on cost for virtually all workloads. Mini's only advantage is the 32K output window (vs 8K for Flash), which matters for code generation or long-form content.
...
For factual lookup, either model works. Budget considerations dominate the decision. If you're processing millions of simple questions, the $0.40 vs $0.075 input difference compounds significantly.
...
**Gemini 2.5 Flash:**
- 100M × (200 × $0.075 + 50 × $0.30) / 1M = 100M × ($0.015 + $0.015) / 1M = $3,000

**GPT-4.1 Mini:**
- 100M × (200 × $0.40 + 50 × $1.60) / 1M = 100M × ($0.08 + $0.08) / 1M = $16,000

Gemini saves $13,000 here. For 1B tickets, Gemini saves $130,000.
...
**Gemini 2.5 Flash:**
- (500 × $0.075 + 2,000 × $0.30) × 10,000 / 1M = $6,375

**GPT-4.1 Mini:**
- (500 × $0.40 + 2,000 × $1.60) × 10,000 / 1M = $34,000

Gemini is 81% cheaper. However, the 32K max output window (vs 8K for Flash) is a key practical difference. Code snippets over 8K tokens are truncated on Flash, requiring multiple API calls and lost context. Mini handles it in one call.
...
Pricing increases from $0.075/$0.30 to $0.40/$1.60, but you gain capacity.
